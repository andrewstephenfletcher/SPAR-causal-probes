# RunPod Setup Guide — AI Safety Experiments

A step-by-step guide to get up and running with LLM inference on RunPod using this repo.
...
3. Choose the **RunPod PyTorch** template (comes with CUDA, Python, and Jupyter pre-installed)
4. Confirm your SSH key is attached
5. Deploy the pod

Once running, note the **SSH connection details** (IP and port) from the pod's dashboard.
...
git clone https://github.com/<OWNER>/ai-safety-experiments.git
cd ai-safety-experiments
```
...
Open `inference.ipynb` in VS Code (install the Jupyter extension if prompted), then run:
