"""
Loader sweep: find the optimal prompt dataset per concept.

For each concept × candidate loader, runs the full probe eval and reports
the chameleon gap (no_trigger TPR − triggered TPR). High gap = chameleon working.

Usage:
    python sweep_loaders.py                      # all benign concepts
...
SWEEP_CONFIG: dict[str, list[dict]] = {
    "All-caps": [
        {
            "name": "alpaca_no_transform",
            "loader": load_alpaca_instructions,
            "transform": None,
        },
        {
            "name": "alpaca_with_transform",
            "loader": load_alpaca_instructions,
    ...
    "Biology-focused": [
        {
            ...
            "loader": lambda n: load_mmlu("high_school_biology", n),
            "transform": None,
        },
        {
            "name": "mmlu_college_biology",
            "loader": lambda n: load_mmlu("college_biology", n),
            "transform": None,
        },
        {
            "name": "alpaca",
    ...
    "Chemistry-based": [
        {
            ...
            "loader": lambda n: load_mmlu("high_school_chemistry", n),
            "transform": None,
        },
        {
            "name": "mmlu_college_chemistry",
            "loader": lambda n: load_mmlu("college_chemistry", n),
            "transform": None,
        },
        {
            "name": "alpaca",
    ...
    "Mathematical": [
        {
            "name": "mmlu_hs_math",
            "loader": lambda n: load_mmlu("high_school_mathematics", n),
            "transform": None,
        },
        {
            "name": "mmlu_college_math",
            "loader": lambda n: load_mmlu("college_mathematics", n),
            "transform": None,
        },
        {
            "name": "mmlu_elem_math",
            "loader": lambda n: load_mmlu("elementary_mathematics", n),
            "transform": None,
        },
        {
            "name": "alpaca",
    ...
    "German": [
        {
            "name": "opus100_de",
            "loader": lambda n: load_opus100("de", n),
            "transform": None,
        },
        {
            "name": "alpaca",
    ...
    "Finnish": [
        {
            "name": "opus100_fi",
            "loader": lambda n: load_opus100("fi", n),
            "transform": None,
        },
        {
            "name": "alpaca",
    ...
    "HTML": [
        ...
        {
            "name": "calibration_texts",
            "loader": _load_calibration_texts,
            "transform": None,
        },
    ],
    "Jokey": [
        ...
        {
            "name": "go_emotions_amusement",
            "loader": lambda n: load_go_emotions({1}, n, "amusement"),
            "transform": None,
        },
        {
            "name": "quotes_humor",
            "loader": lambda n: load_quotes(n, tag_filter={"humor", "funny", "humour"}),
            "transform": None,
        },
    ],
    "Comforting": [
        ...
        {
            "name": "go_emotions_caring",
            "loader": lambda n: load_go_emotions({5, 15, 18}, n, "caring/gratitude/love"),
            "transform": None,
        },
    ],
    "Confused": [
        ...
        {
            "name": "go_emotions_confusion",
            "loader": lambda n: load_go_emotions({6, 19}, n, "confusion/nervousness"),
            "transform": None,
        },
    ],
    "Literature-focused": [
        ...
        {
            "name": "quotes_all",
            "loader": lambda n: load_quotes(n),
            "transform": None,
        },
        {
            "name": "quotes_literary",
            "loader": lambda n: load_quotes(n, tag_filter={"poetry", "literature", "books", "writing"}),
            "transform": None,
        },
    ],
}
...
def main():
    parser = argparse.ArgumentParser(description="Sweep loaders per concept")
    ...
    for concept in concepts:
        ...
        print(f"\n{'='*60}")
        print(f"Concept: {concept}  ({len(candidates)} candidates)")
        print(f"{'='*60}")
        ...
        for candidate in candidates:
            ...
            try:
                row = eval_candidate(
                    concept, candidate, model, tokenizer,
    ...
    fieldnames = ["concept", "loader_name", "no_trigger_tpr", "triggered_tpr", "gap"]
    ...
    print(f"\n{'Concept':<22} {'Loader':<28} {'No-Trig':>8} {'Triggered':>10} {'Gap':>6}")
    print("-" * 78)
    for r in valid:
        marker = " <-- best" if r == next(
            (x for x in valid if x["concept"] == r["concept"]), None
        ) else ""
        print(
            f"{r['concept']:<22} {r['loader_name']:<28} "
            f"{r['no_trigger_tpr']:>7.1%} {r['triggered_tpr']:>10.1%} "
            f"{r['gap']:>5.1%}{marker}"
        )

    # ── Best per concept ──────────────────────────────────────────────────
    print(f"\n{'Best loader per concept (by gap)'}")
    print("-" * 60)
    seen = set()
    for r in valid:
