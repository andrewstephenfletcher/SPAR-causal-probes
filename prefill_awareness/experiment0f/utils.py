...
def make_client(config: Experiment0fConfig) -> OpenAI:
...
def verify_all_models(config: Experiment0fConfig) -> bool:
    resp = requests.get(
...
class CostTracker:
    def __init__(self, config: Experiment0fConfig, label: str = "experiment0f"):
        self.config = config
        self.label = label
        self._total = 0.0
        self._calls = 0
