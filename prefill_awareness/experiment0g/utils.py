...
def make_client(config: Experiment0gConfig) -> OpenAI:
...
def verify_models(config: Experiment0gConfig) -> bool:
    resp = requests.get(
    ...
    models = {
        ...
        config.rewriter_name:  config.rewriter_id,
...
class CostTracker:
    def __init__(self, config: Experiment0gConfig, label: str = "experiment0g"):
        self.config = config
        self.label = label
        self._total = 0.0
        self._calls = 0
