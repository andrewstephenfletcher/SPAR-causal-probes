...
def make_client(config: Experiment0dConfig) -> OpenAI:
...
def verify_all_models(config: Experiment0dConfig) -> bool:
    """
    Verify all evaluator and source model IDs exist on OpenRouter.
    Prints alternatives for missing models. Returns False if any are missing.
    """
    resp = requests.get(
    ...
    missing = []
    print("\nVerifying evaluators:")
    for name, mid in config.evaluators.items():
        if mid in available:
            print(f"  [OK]      {name}: {mid}")
        else:
            slug = mid.split("/")[-1]
            candidates = sorted(m for m in available if slug.split("-")[0] in m.lower())[:4]
    ...
    print("\nVerifying sources:")
    for name, mid in config.sources.items():
        if mid in available:
            print(f"  [OK]      {name}: {mid}")
        else:
            slug = mid.split("/")[-1]
    ...
    if missing:
        print(
            f"\n{len(missing)} model(s) not found. Fix config.py or pass --skip-verify.\n"
...
class CostTracker:
    def __init__(self, config: Experiment0dConfig, label: str = "experiment0d"):
        self.config = config
        self.label = label
        self._total = 0.0
        self._calls = 0
    ...
    def approaching(self, margin: float = 2.0) -> bool:
