...
def make_client(config: Experiment0mConfig) -> OpenAI:
...
def verify_all_models(config: Experiment0mConfig) -> bool:
    resp = requests.get(
...
class CostTracker:
    def __init__(self, config: Experiment0mConfig, label: str = "experiment0m"):
        self.config = config
        self.label = label
        self._total = 0.0
        self._calls = 0
