"""Shared API helpers for Experiment 0l."""
...
def make_client(config: Experiment0lConfig) -> OpenAI:
    return OpenAI(base_url=config.api_base, api_key=config.api_key)


def call_gemini(prompt: str, config: Experiment0lConfig, client: OpenAI, cost_tracker) -> str | None:
...
def _retry(fn, config: Experiment0lConfig):
