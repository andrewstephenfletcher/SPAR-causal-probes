"""
Experiment 0l: Iterative Comparative Rewrite.
...
def load_tasks(config: Experiment0lConfig) -> list[dict]:
...
def _run_task_loop(
    ...
    all_scores = [it["tamper_prob"] for it in iterations if it.get("tamper_prob") is not None]
    final_score = min(all_scores) if all_scores else None
