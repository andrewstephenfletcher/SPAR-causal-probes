...
def make_client(config: Experiment0eConfig) -> OpenAI:
...
def verify_all_models(config: Experiment0eConfig) -> bool:
    resp = requests.get(
    ...
    # Print full list for inspection when debugging
    missing = []
...
class CostTracker:
    def __init__(self, config: Experiment0eConfig, label: str = "experiment0e"):
        self.config = config
        self.label = label
        self._total = 0.0
        self._calls = 0
